...
export default function SalesDashboard() {
  ...
  return (
    <div className="app-layout">
      ...
      <button className="hamburger" onClick={() => setSidebarOpen(v => !v)}><i className="bi bi-list" /></button>
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          <div className="nav-item active"><i className="bi bi-calculator" /> Incentive Calculator</div>
        </nav>
        <div className="sidebar-footer">
          <button className="btn-back" onClick={() => navigate('/')}><i className="bi bi-arrow-left" /> Back to Home</button>
        </div>
      </aside>
      ...
      <main className="main-content">
        {!isSupabaseConfigured && (
          <div className="alert-demo">
            ...
            <strong>Demo Mode:</strong> Data resets on refresh. Connect Supabase via <code>.env</code> to persist.
          </div>
        )}
        ...
        <div className="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
          <div>
            ...
            <p>Log vehicle sales for the selected month and view real-time incentive breakdown.</p>
          </div>
          <div className="d-flex align-items-center gap-2">
            <label style={{ fontWeight: 600, fontSize: '.85rem', color: 'var(--muted)' }}>Month:</label>
          ...
          <div className="row g-3">
            ...
            <div className="col-lg-7">
              <div className="table-card">
                ...
                {saveMsg && (
                  <div style={{ background: '#f0fdf4', color: '#166534', padding: '.6rem 1.25rem', borderBottom: '1px solid var(--border)', fontSize: '.85rem' }}>
                    <i className="bi bi-check-circle me-1" /> {saveMsg}
                  </div>
                )}
                {models.length === 0 ? (
                  <div className="text-center p-4 text-muted">
                    <i className="bi bi-car-front" style={{ fontSize: '2rem' }} />
                    <p className="mt-2">No vehicle models found. Ask your admin to add models.</p>
                  </div>
                ) : (
                    ...
                    <table className="table table-hover mb-0">
                          ...
                          <tr key={m.id}>
                            ...
                            <td className="text-center">
                              <div className="d-flex align-items-center justify-content-center gap-2">
                                <button style={{ border: 'none', background: '#f1f5f9', borderRadius: 6, width: 28, height: 28, cursor: 'pointer', fontWeight: 700 }}
                                ...
                                <button style={{ border: 'none', background: '#dbeafe', borderRadius: 6, width: 28, height: 28, cursor: 'pointer', fontWeight: 700, color: 'var(--primary-light)' }}
                                  onClick={() => setQty(m.id, (quantities[m.id] || 0) + 1)}>+</button>
                              </div>
                            </td>
                          </tr>
                      ...
                      <tfoot>
                        <tr style={{ background: '#f8fafc' }}>
                          ...
                          <td className="text-center"><strong style={{ fontSize: '1.1rem' }}>{totalQty}</strong></td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
            ...
            <div className="col-lg-5">
              <div className="incentive-panel mb-3">
                ...
                <div className="incentive-row">
                  ...
                  {slab ? (
                    <span className="slab-active">
                      <i className="bi bi-star-fill" style={{ fontSize: '.7rem' }} />
                ...
                <div className="incentive-row">
                  ...
                  <span style={{ fontWeight: 700 }}>{rate ? fmt(rate) : '—'}</span>
                </div>
                <div className="incentive-row">
                  <span>Calculation</span>
                  <span style={{ opacity: .75, fontSize: '.85rem' }}>
                  ...
                  <table className="table mb-0" style={{ fontSize: '.85rem' }}>
                    ...
                    <tbody>
                      {slabs.map(s => {
                        ...
                        return (
                          <tr key={s.id} style={{ background: isActive ? '#fffbeb' : undefined }}>
                            <td>{s.label || `${s.min_quantity}${s.max_quantity ? `–${s.max_quantity}` : '+'} cars`}</td>
                            <td style={{ color: 'var(--accent)', fontWeight: 700 }}>{fmt(s.incentive_per_car)}</td>
                            <td>{isActive ? <span className="badge bg-warning text-dark">Active ✓</span> : <span className="badge bg-light text-muted">—</span>}</td>
                          </tr>
                        );
                      })}
